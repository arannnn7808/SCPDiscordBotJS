...
describe('Translate Command', () => {
    ...
    test('should translate text successfully', async () => {
        ...
        translateMock.mockResolvedValue({ text: 'Hola mundo', from: { language: { iso: 'en' } } });

        const francMock = require('franc').franc;
        francMock.mockReturnValue('eng');

        mockInteraction.options.getString.mockReturnValueOnce('Hello world').mockReturnValueOnce('es');
    ...
    test('should handle unknown source language', async () => {
        const translateMock = require('@vitalets/google-translate-api').translate;
        translateMock.mockResolvedValue({ text: 'Translated text', from: { language: { iso: 'und' } } });

        const francMock = require('franc').franc;
        francMock.mockReturnValue('und');

        mockInteraction.options.getString.mockReturnValueOnce('Unknown language text').mockReturnValueOnce('es');
